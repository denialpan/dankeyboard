...
**Only five files** are created that the program read and writes, excluding itself. They can be found in the `.\dankeyboard_data` or navigating to `File -> Open Current Data Directory`:
...
**All above files listed and its respective data can be read, modified, or deleted. Deletion of `.\dankeyboard_data` removes all data that the program has generated and modified. You are free to delete `dankeyboard.exe` at any time without any system issues relating to above `.dll` files.** 

_Depending on the type of package you download, there will be extraneous files. More information here._
...
To ensure that you download a safe version, only download from this repository's releases page or from one of the links below. Each latest release will have its respective `SHA256` hash listed here as well:
